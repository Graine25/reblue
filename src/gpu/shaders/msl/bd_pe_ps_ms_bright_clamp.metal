...
// Hand-maintained MSL port of hlsl/bd_pe_ps_ms_bright_clamp.hlsl. The body is a
// line-for-line copy of that file's, so the two stay diffable; the only edits
// are the MSL-required ones (explicit bool->float, output struct, no [branch]).

#include "bd_pe_ps_common.metali"

#define g_vCount BUFFER_LOAD_FLOAT4(g_PushConstants.PixelShaderConstants + 416)
#define g_vSampleOffsets(INDEX)                                                \
    select((INDEX) < 224,                                                      \
           BUFFER_LOAD_FLOAT4(g_PushConstants.PixelShaderConstants +           \
                              (0 + min(INDEX, 223)) * 16),                     \
           float4(0.0))
#define g_vSampleWeights(INDEX)                                                \
    select((INDEX) < 211,                                                      \
           BUFFER_LOAD_FLOAT4(g_PushConstants.PixelShaderConstants +           \
                              (13 + min(INDEX, 210)) * 16),                    \
           float4(0.0))
...
[[fragment]] PixelShaderOutput shaderMain(BD_PE_PS_PARAMS)
{
	...
	r1.xyz = float3(-abs(r0.xxx) > c253.zzz);
	p0 = g_vCount.x > 0.0;
	ps = p0 ? 0.0 : 1.0;
	if (p0)
	{
	...
	shaderOutput.oC0.xyz = max(r1.xyz, r1.xyz);
	...
	if (g_SpecConstants() & SPEC_CONSTANT_ALPHA_TEST)	{		clip(shaderOutput.oC0.w - g_AlphaThreshold);
	}
	return shaderOutput;
}
